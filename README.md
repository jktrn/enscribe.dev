...
- I use a rich typographic system with [Utopia](https://utopia.fyi/) type scales, old-style numerals, smart punctuation, muted parentheticals, and automatic small caps for acronyms and abbreviations. The numerals and small caps are baked into the served font files rather than applied with `font-variant`, so canvas text measurement sees the same glyph widths the page renders (the line-breaking algorithm depends on this).
