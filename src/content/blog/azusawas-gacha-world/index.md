...
<static-tweet>
<tweet-header>
<img src="https://pbs.twimg.com/profile_images/1516810637858545668/UKYxlCqh_400x400.jpg" alt="" width="48" height="48" />
<tweet-author>
<b>Project Sekai CTF</b>
<span>@ProjectSEKAIctf</span>
<span>August 7, 2023</span>
</tweet-author>
<a href="https://twitter.com/ProjectSEKAIctf/status/1688745825504268288">View on Twitter</a>
</tweet-header>
...
<video controls src="/blog/azusawas-gacha-world/preview.mp4"></video>
...
![Genshin Impact Pity](./assets/genshin-pity-dark.svg)
...
![Project Sekai Gacha](./assets/project-sekai-gacha-dark.webp)
...
![SekaiCTF 2022 Pie Chart](./assets/2022-pie-chart-dark.svg)
...
![Brainstorming I](./assets/brainstorming-1-dark.webp)
...
![Brainstorming II](./assets/brainstorming-2-dark.webp)
...
Here is `src/main.ts{:file}`, the main entrypoint of the server and the place where the pity system, gacha pool, and endpoint are implemented:

~~~ts title="`src/main.ts{:file}`"
...
Testing this endpoint was made buttery smooth with the help of [Jest](https://jestjs.io/); I made a test suite in `__tests__/main.test.ts{:file}` which sent a variety of different inputs, and made sure that a correct response was being outputted:

~~~ts title="`__tests__/main.test.ts{:file}`"
...
<static-tweet>
<tweet-header>
<img src="https://pbs.twimg.com/profile_images/1340744369976930304/tVcKoFVd_400x400.jpg" alt="" width="48" height="48" />
<tweet-author>
<b>Eana Hufwe</b>
<span>@blueset</span>
...
<img src="https://pbs.twimg.com/media/E-Fh71rXoAADJ4b?format=png&name=900x900" alt="Tweet media" />
...
For this section of the blog post, I'll be going over each individual script located within `Assets/Scripts/{:dir}`—conveniently, structuring the post in this manner actually fleshes out a lot of my challenge's functionality, and places the various scripts in context for later when we talk about reverse-engineering. Each script will have its own dedicated section, ordered by how important I believe they are. Let's get started!

---

### `GachaManager.cs{:file}`

Of course, in order for the entire gacha system to function, we needed a script which would facilitate interacting with the backend server. This script is responsible for crafting and sending the proper `POST` request to the server, and delegating the parsed response to the `UIManager.cs{:file}` script for display. We first define a `RequestClasses{:cs}` namespace, which defines the structure of the JSON payloads that we will be sending/receiving:

~~~cs title="`Assets/Scripts/RequestClasses.cs{:file}`"
...
~~~cs title="`Assets/Scripts/GachaManager.cs{:file}`"
...
~~~cs title="`Assets/Scripts/GachaManager.cs{:file}`" mark={4} startLineNumber=57
    UnityWebRequest CreateGachaWebRequest(string json)
    {
        byte[] bodyRaw = System.Text.Encoding.UTF8.GetBytes(json);
        string requestUrl = "aHR0cDovLzE3Mi44Ni42NC44OTozMDAwL2dhY2hh";
...
~~~cs title="`Assets/Scripts/GachaManager.cs{:file}`" startLineNumber=46
            else
            {
                uiManager.GenericModalHandler(
                    uiManager.failedConnectionModal,
...
~~~cs title="`Assets/Scripts/GachaManager.cs{:file}`" mark={7} collapse="9-36" startLineNumber=38
            if (request.result == UnityWebRequest.Result.Success)
            {
                HandleGachaResponse(request.downloadHandler.text, numPulls);
                GachaResponse response = JsonUtility.FromJson<GachaResponse>(
...
~~~cs title="`Assets/Scripts/GameState.cs{:file}`"
using UnityEngine;
...
~~~cs title="`Assets/Scripts/UIManager.cs{:file}`"
...
I know this looks intensely overwhelming, but it's just because all of these different, uncorrelated methods are all thrown into single file because they all have to do with UI; it's not actually that bad if you chunk it up to make it more digestible. I'll attempt to briefly explain each of these methods (which is just... sheer ridiculousness 🤣):
...
Used in the volume sliders in `menuModal{:cs}`. Utilizes the `AudioController{:cs}` (which we'll go over later) to adjust sounds:

![Volume Sliders](./assets/volume-sliders.png)

Also plays a little sound effect which changes pitch depending on the direction the slider is moving. Of course, since we don't want the sound effect to be spammed per tick of the slider, I added a cooldown.
...
`public void OnPullButtonClick(int cost, int numPulls){:cs}`

</summary>

Opens the `pullConfirmationModal{:cs}` and changes the various TextMeshPro objects inside to be accurate (changes the color of `currentCrystalsText{:cs}` to red and disables `pullConfirmationContinueButton{:cs}` if the user doesn't have enough crystals):

![Confirmation Modal](./assets/confirmation-modal.png)
...
`public void GenericModalHandler(GameObject modal, Button closeButton){:cs}`

</summary>

Plays the default zoom-in/zoom-out animations, sound effects, and enables darkened background whenever a modal is opened via button:

![Modal Handler](./assets/modal-handler.gif)
...
`public IEnumerator DisplaySplashArt(Character[] characters){:cs}`
...
`private IEnumerator DisplayTwoStarCharacter(Character character){:cs}`
...
`private IEnumerator DisplayThreeStarCharacter(Character character){:cs}`

</summary>

Handles three-star splash art. Since splash art for this rarity takes up the entire screen instead of using a small silhouette, I had to give it different treatment (and therefore different animations):

![Three Star Animation](./assets/three-star-animation.gif)
...
`private IEnumerator DisplayFourStarCharacter(Character character){:cs}`
...
`private IEnumerator SetInactiveAfterDelay(GameObject gameObject, float delay){:cs}`
...
`private void UpdateSplashArtImage(string splashArt, Image image){:cs}`
...
`private void UpdateSilhouetteImage(string splashArt){:cs}`
...
`private void DisplayAvatars(Character[] characters){:cs}`

</summary>

Handles the instantiation/destruction of `avatarImages{:cs}` within the Grid Layout Group component of `gachaOverviewCanvas{:cs}`:

![Gacha Overview](./assets/gacha-overview.gif)
...
`private void SetImageOpacity(MaskableGraphic image, float targetOpacity){:cs}`
...
`private bool CheckForSkipOrClick(Character[] characters, int index){:cs}`
...
The `AudioController.cs{:file}` itself first needs a class `Sound{:cs}`, which defines an `AudioClip{:cs}` with a name:

~~~cs title="`Assets/Scripts/Sound.cs{:file}`"
using UnityEngine;
...
~~~cs title="`Assets/Scripts/AudioController.cs{:file}`"
using System.Collections;
using System.Collections.Generic;
using UnityEngine;
using System;
...
With this, we can now freely drag audio clips into the audio controller, rename them, and call them on a whim from any of our other scripts, like our `UIManager.cs{:file}`:

![Audio Inspector](./assets/audio-inspector.png)

~~~cs title="`Assets/Scripts/UIManager.cs{:file}`" mark={14,16} collapse="3-12" startLineNumber=433
    private void DisplayGachaOverview()
    {
        if (gameState.crystals < 1000)
        {
...
~~~cs title="`Assets/Scripts/BackgroundScroller.cs{:file}`"
using UnityEngine;
using UnityEngine.UI;
...
~~~cs title="`Assets/Scripts/ShrinkButton.cs{:file}`"
using System.Collections;
using UnityEngine;
using UnityEngine.UI;
using UnityEngine.EventSystems;
...
### `CameraScaler.cs{:file}`, `FullscreenHandler.cs{:file}`
...
~~~cs title="`Assets/Scripts/CameraScaler.cs{:file}`"
using UnityEngine;
...
~~~cs title="`Assets/Scripts/FullscreenHandler.cs{:file}`"
using System.Collections;
using System.Collections.Generic;
using UnityEngine;
...
<challenge-info>
<dl>
<div><dt>Author</dt><dd>enscribe</dd></div>
...
<div><dt>Solves</dt><dd>250</dd></div>
<div><dt>File</dt><dd>
...
We're initially provided with a `dist/{:dir}` folder with the following structure:

:::file-tree
- dist/
  - Asusawa's Gacha World_Data/
  - MonoBleedingEdge/
...
Let's use [dnSpy v6.1.8](https://github.com/dnSpy/dnSpy) to our advantage here to reverse engineer the game. Since all Unity-compiled C# code goes into an `Assembly-CSharp.dll{:file}` within the `dist/Asusawa's Gacha World_Data/Managed` directory, we should be analyzing this file first and foremost. Opening dnSpy and dragging the DLL into the program will reveal the following structure:

![Initial dnSpy](./assets/initial-dnspy.png)

Sifting through the code, the most relevant script out of all seems to be a `GachaManager.cs{:file}`, which creates `POST` requests to a backend server of some kind. We find a suspicious string near the top of the `CreateGachaWebRequest(){:cs}` method:
...
~~~cs title="`Assembly-CSharp.dll{:file}`" mark={9,10} startLineNumber=51
...
~~~cs title="`Assembly-CSharp.dll{:file}`" mark={4} startLineNumber=27
...
~~~cs title="`Assembly-CSharp.dll{:file}`" ins={14} del={13,30-32}
using System;
using System.Collections;
using RequestClasses;
using UnityEngine;
