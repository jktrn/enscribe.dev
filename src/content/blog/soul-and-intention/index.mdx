...
<StaticTweet
  ...
  username="Jovan - StarvingHustler.com"
...
Although they may look very different <span class="text-muted-foreground">(exclusively because my design skills improved throughout this entire process)</span> in general I followed several mantras through every iteration:
- **My homepage should show character.** Although I used to do this with the way I write, eventually this ended up manifesting as an actual character <span class="text-muted-foreground">(the sepia-colored cartoon guy you see throughout my blog, which we'll call a "bean guy").</span> This bean guy caricature of myself is meaningful because I used to draw him while doodling in middle school, since I had no idea how to properly draw a human <span class="text-muted-foreground">(and because it looks cute).</span> This also brings up the point that meaning, even when personal and unbeknownst to anyone else, provides a lot of value**. [TODO: Talk about how the face is intentionally short, size of the mouth, size of the eyes, etc]**
- **My homepage should quickly introduce myself.** I used to do this with a small paragraph blurb <span class="text-muted-foreground">(which is what every personal homepage does)</span>, but when I was making the bento box I really felt like it looked like a cartoon panel, so I opted for the approach of having text bubbles come out of the bean guy to emulate me "talking" to the user, which makes it feel a tad bit more personal.
...
- **My homepage should be engaging.** If we regress back to the SF tech-bro mentality, the goal of a homepage is to get the user to interact and move somewhere else that retains them on the site (a.k.a. exploring). All of the above help increase retention <span class="text-muted-foreground">(you need to stay a bit longer to comprehend and explore the graphs, micro-interactions, hover efffects, etc.)</span>, but I additionally include social links that will further coax the user into doing something insidious like following me on [Twitter](https://twitter.com/enscry) and [GitHub](https://github.com/jktrn) <span class="text-muted-foreground">(do that please, you're already here!)</span>.
