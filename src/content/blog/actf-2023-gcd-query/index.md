...
<div><dt>Author</dt><dd>skittles1412</dd></div>
<div><dt>Category</dt><dd>

`PPC`

</dd></div>
<div><dt>Points</dt><dd>475</dd></div>
<div><dt>Solves</dt><dd>43</dd></div>
<div><dt>Flag</dt><dd>

<challenge-flag>`amateursCTF{probabilistic_binary_search_ftw}`</challenge-flag>
...
We're initially provided with an attachment `main.py{:file}` and a remote server `amt.rs:31692`. The server component contains the following:

~~~py title="`main.py{:file}`"
...
To recover `x{:py}`, we need to choose `n{:py}` and `m{:py}` so that each returned GCD gives us a useful constraint on `x{:py}`. We can then combine those constraints to determine the integer before the server asks for our guess.
...
### The modular arithmetic nerd's favorite carnival game

![Carnival Game](./assets/carnival-visual.webp)
...
![Bob Handing Papers](./assets/handing-papers.webp)
...
<dl>
<div><dt>Solver</dt><dd><a href="https://github.com/jktrn"><img src="https://github.com/jktrn.png" alt="" width="16" height="16" />enscribe</a></dd></div>
<div><dt>Author</dt><dd>hellopir</dd></div>
<div><dt>Category</dt><dd>

`PPC`

</dd></div>
<div><dt>Points</dt><dd>481</dd></div>
<div><dt>Solves</dt><dd>34</dd></div>
<div><dt>Flag</dt><dd>

<challenge-flag>`amateursCTF{crt_really_is_too_op...wtf??!??!?!?must_be_cheating!!...i_shouldn't've_removed_query_number_cap.}`</challenge-flag>
...
~~~py title="`solve.py{:file}`" ins={11,18} del={10,13,17}
from sympy.ntheory.modular import crt
from sympy import primorial
from tqdm import tqdm
from pwn import *
...
~~~py title="`solve.py{:file}`" ins={1-2} del={3-4} startLineNumber=6
# Generate the 256th primorial
N = 2**8
