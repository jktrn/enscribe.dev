...
const styleOf = (overrides: Partial<Record<string, string>> = {}) => {
  ...
  // Supply computed font properties that Happy DOM does not implement yet.
  return new Proxy(document.createElement("span").style, {
    get: (style, property) =>
      typeof property === "string" && Object.hasOwn(values, property)
        ? values[property]
        : Reflect.get(style, property),
  })
}
...
describe("the letterspacing one line declaration can carry", () => {
  const readerOf = (values: readonly string[]) => {
    const elements = values.map(() => document.createElement("span"))
    const styles = new Map(
      elements.map((element, index) => [
        element as Element,
        styleOf({ letterSpacing: values[index] }),
      ]),
    )
    const read = (element: Element) => styles.get(element)!
